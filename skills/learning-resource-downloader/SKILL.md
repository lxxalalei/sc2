...
description: 根据用户在候选清单中的选择下载学习资源到工作缓存目录。用于 selector 输出 learning-resource-selection/v1 或 source skill 输出 learning-resource-candidate/v1 后，下载 PDF、DOC、PPT、图片、m3u8 视频等可访问资源；不负责搜索、评分、最终归档或资料库整理。
---
...
接收 `learning-resource-selector` 输出的选择清单，或 source skill 直接输出的候选清单，再根据用户选择将可下载资源保存到工作缓存目录，并输出标准下载结果。
...
输入可以是 `learning-resource-selection/v1` 和用户选择：
...
也可以直接接收 `learning-resource-candidate/v1`：
...
直接接收候选 JSON 时，下载器会临时把候选映射为 `1/2/3...` 选项。这样模型从 `smartedu-resources --fetch-details` 拿到真实文件候选后，可以跳过 selector 的人工确认层，直接下载用户明确要求的“全部资料”或指定类型资源。
...
下载器支持普通 m3u8 分片合并。对 SmartEdu 标准 AES-128 加密 m3u8，会按平台前端流程获取 nonce/sign、解出视频 key，再解密 TS 分片并合并输出 `.ts`。DRM、验证码、二次验证和未识别站点专用签名仍不处理。
...
直接下载 source skill 输出的候选 JSON：

```bash
python3 skills/learning-resource-downloader/scripts/download_selected.py \
  smartedu-file-candidates.json \
  --select all \
  --allow-auth \
  --work-dir .learning-resource-work
```
