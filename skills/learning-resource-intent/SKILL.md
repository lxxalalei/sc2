...
将家长或儿童提出的自然语言学习需求转换为结构化学习主题与搜索计划。先澄清“孩子是谁、要学什么、为了什么学、需要什么形式的资源、在哪个场景使用”。

本 skill 不负责搜索、评分、下载或归档文件，只负责为来源类 skill 和评分流程准备结构化需求。

## 通用学习需求原则

优先围绕“学习主题、适龄性、学习目标、资源形式和使用场景”分析需求。资源类型只是输出槽位和后续检索约束，不驱动默认流程。
