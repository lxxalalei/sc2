...
`--list-only` 输出符合 `learning-resource-candidate/v1` 候选资源契约，可直接交给 `learning-resource-analyzer` 和 `learning-resource-ranker` 使用。
...
- `candidate_schema`: `--list-only` 模式下为 `learning-resource-candidate/v1`
- `candidates`: `--list-only` 和下载模式下的标准候选资源列表
...
- `source`, `source_name`, `source_url`, `resource_id`
- `title`, `resource_type`, `format`
- `stage`, `grade`, `subject`, `version`, `volume`, `topic`
- `provider`, `official`, `downloadable`, `requires_auth`
- `size`, `metadata_confidence`, `raw`
- `local_file`: 下载成功后指向最终资料库中的本地 PDF，供详情分析 skill 使用

## 与 analyzer/ranker 串联
...
python3 ../learning-resource-analyzer/scripts/analyze_candidates.py \
  /tmp/smartedu-candidates.json -o /tmp/smartedu-analyzed.json

python3 ../learning-resource-ranker/scripts/rank_candidates.py \
  /tmp/smartedu-analyzed.json
```

下载成功后，输出候选会包含 `local_file`，可再次交给 analyzer 对本地 PDF 做页数等轻量分析。
