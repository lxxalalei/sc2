...
学习主题覆盖课内和课外资源，包括四则运算、拼音识字、儿童百科、唐诗宋词、儿歌、绘本、科学启蒙、艺术启蒙、逻辑思维、课件、习题、试卷、图片、音频、视频、网页资料等。教材只是其中一种资源类型，不作为默认判断起点。
...
```text
local-library-search -> optimized source skills -> web fallback
  -> learning-resource-analyzer -> learning-resource-ranker -> learning-resource-selector
```
...
负责在搜索前理解和澄清用户需求，输出结构化意图。它不搜索、不下载、不归档，只判断用户需求是否足够明确，并生成搜索计划。当前采用通用学习需求模型，优先分析孩子年龄/阶段、核心学习主题、学习目标、资源形式和使用场景。
...
- “给 8 岁孩子找可打印四则混合运算练习题” -> 生成本地优先、多来源候选查询计划。
- “帮孩子找恐龙百科视频和图文资料” -> 生成多格式资源搜索计划。

### `smartedu-resources`

位置：[skills/smartedu-resources](skills/smartedu-resources)

负责统一分析国家中小学智慧教育平台站内资源，例如课程教学、备课资源、精品课、习题、试卷、实验教学、德育、家庭教育、课后服务、专题、图片、音频、视频、教材等。第一版只输出标准候选，不批量下载、不写资料库。

早期教材兼容脚本说明见：[docs/smartedu-textbooks-usage.md](docs/smartedu-textbooks-usage.md)
